  ...
  return (
    ...
    >
      ...
      <div className="panel-header">
        <h2>Layers</h2>
        <button type="button" className="btn-icon" title="Add layer" onClick={() => setShowLayerModal(true)}>
          +
        </button>
      </div>
        ...
        {layers.map((layer) => (
          <li key={layer.id} className={selectedLayerId === layer.id ? 'active' : ''}>
            <div className="layer-row">
              <button type="button" className="layer-btn" onClick={() => onSelectLayer(layer.id)}>
              ...
              <ul className="file-list">
                {files.map((file) => (
                  <li key={file} className={selectedFile === file ? 'active' : ''}>
                    <button type="button" className="file-btn" onClick={() => onSelectFile(file)}>
                      {file}
                    </button>
                    <button
                      type="button"
                ...
                <li className="file-actions">
                  <button type="button" className="add-file-btn" onClick={() => setShowFileModal(true)}>
                    + New file
                  </button>
                  <button
                    type="button"
      ...
      {showLayerModal && (
        <div className="modal-overlay" onClick={() => setShowLayerModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label>
              ID (lowercase)
              <input value={layerId} onChange={(e) => setLayerId(e.target.value)} placeholder="e.g. persona" />
            </label>
            <label>
              Name
              <input value={layerName} onChange={(e) => setLayerName(e.target.value)} placeholder="e.g. Persona" />
            </label>
            <label>
              Description
              <input value={layerDesc} onChange={(e) => setLayerDesc(e.target.value)} placeholder="Optional" />
            </label>
            <div className="modal-actions">
              <button type="button" onClick={() => setShowLayerModal(false)}>Cancel</button>
              <button type="button" className="btn-primary" onClick={handleCreateLayer}>Create</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showFileModal && (
        <div className="modal-overlay" onClick={() => setShowFileModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button type="button" onClick={() => setShowFileModal(false)}>Cancel</button>
              <button type="button" className="btn-primary" onClick={handleCreateFile}>Create</button>
            </div>
          </div>
        </div>
      )}
